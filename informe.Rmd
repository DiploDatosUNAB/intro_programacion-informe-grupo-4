...
\*FILTRO PARA VER LA PARTICIPACIÓN POR GENERO EN EL SECTOR INDUSTRIAL

```{r}
empleo_industria_mercosur <- select(empleo_genero_ordenado, pais_region, Anio, empleo_industria_mujeres, empleo_industria_hombres) |>
  filter(pais_region %in% c("Argentina", "Uruguay", "Paraguay", "Brasil", "Venezuela", "Bolivia" ))|> 
  filter (Anio >= 1998)
```

\*FILTRO PARA VER LA PARTICIPACIÓN POR GENERO EN EL SECTOR SERVICIOS

```{r}
empleo_servicios_mercosur <- select(empleo_genero_ordenado, pais_region, Anio, empleo_servicios_mujeres, empleo_servicios_hombres) |>
  filter(pais_region %in% c("Argentina", "Uruguay", "Paraguay", "Brasil", "Venezuela", "Bolivia" ))|> 
  filter (Anio >= 1998)
```
...
```{r}
industria_2018 <- select(empleo_genero_ordenado, pais_region, Anio, empleo_industria_mujeres, empleo_industria_hombres)|>
  filter(pais_region %in% c("Argentina", "Uruguay", "Paraguay", "Brasil", "Venezuela", "Bolivia" ))|>
  filter(Anio == 2018) |> 
...
```
```{r}
servicios_2018 <- select(empleo_genero_ordenado, pais_region, Anio, empleo_servicios_mujeres, empleo_servicios_hombres)|>
  filter(pais_region %in% c("Argentina", "Uruguay", "Paraguay", "Brasil", "Venezuela", "Bolivia" ))|>
  filter(Anio == 2018) |> 
...
\*Del grafico presentado podemos leer que nuestra hipotesis se confirma: el peso del sector industrial como empleador de la fuerza laboral masculina en los paises del Mercosur es mucho mayor que en el caso de la fuerza laboral femenina. Lo contrario ocurre al analizar el sector servicios. Entendemos que esto se debe al fenomeno llamado "segmentación vertical". Es decir, el hecho de que existan actividades economicas que emplean mayormente a varones, denominadas "masculinizadas" y actividades que emplean mayormente a mujeres, denominadas "feminizadas". 
...
\*A partir de la lectura del grafico podemos concluir que en el periodo 1998 - 2018 no solo no crecio el acceso a empleos en la industria de las mujeres sino que disminuyó. La tasa de variacion en todos los casos es negativa, lo cual nos muestra que el porcentaje de mujeres que tienen empleos dentro del sector industrial se redujo.
