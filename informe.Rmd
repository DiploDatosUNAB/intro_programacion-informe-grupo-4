...
\*Para responder a la pregunta, calculamos el promedio para cada uno de los años, de del porcentaje dentro de la fuerza laboral masculina de los paises del Mercosur que se encuentra empleado en la industria. Lo mismo hicimos para el caso de las mujeres.

\* Filtré los datos existentes para los paises del mercosur dentro de los años que van de 1998 en adelante.

```{r}
empleo_mercosur <- empleo_genero_ordenado |> 
  filter(pais_region == "Argentina" | pais_region == "Uruguay" | pais_region == "Paraguay" | pais_region == "Brasil" | pais_region == "Venezuela") |> 
  filter (Anio >= 1998)
```
\* Para responder la primera pregunta compararemos el promedio de varones y mujeres empleadxs en el sector industrial en los ultimos 20 años dentro de los paises del mercosur y compararemos los resultados.
```{r}
promedio_industria_x_genero <- empleo_mercosur |>
...
Calcule el promedio agrupado por año para todos los datos existentes dentro de la variable industria mujeres e industria hombres

```{r}
promedio_industria_x_anio <- empleo_mercosur |>
  group_by(Anio) |>
  summarise(promedio_industria_hombres = mean(empleo_industria_hombres),
            promedio_industria_mujeres = mean(empleo_industria_mujeres))
```
...
```{r}
ggplot(data=promedio_industria_x_anio, mapping = aes( x=Anio, y=promedio_industria_hombres)) + 
  geom_col() +
  geom_line()
  
```
Calculé el promedio agrupado por pais para todos los datos existentes dentro de la variable industria mujeres e industria hombres en el periodo 1998 - 2018. 
```{r}
promedio_industria_x_pais <- empleo_mercosur |>
  group_by(pais_region) |>
  summarise(promedio_industria_hombres = mean(empleo_industria_hombres),
            promedio_industria_mujeres = mean(empleo_industria_mujeres))
```
...
##Para contestar nuestra segunda pregunta queremos ver especificamente que sucede en los años de los extremos, con el objetivo de poder comparar el comportamiento de ambas variables y evaluar si hubo una reduccion o ampliacion de la brecha existentes. Por esta razon filtraremos los años 1998 y 2018.
```{r}
  promedio_industria_1998_2018 <- empleo_mercosur |>
  group_by(Anio) |>
