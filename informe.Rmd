...
empleo_mercosur <- select(empleo_genero_ordenado, pais_region, Anio, empleo_industria_mujeres, empleo_industria_hombres) |>
  filter(pais_region == "Argentina" | pais_region == "Uruguay" | pais_region == "Paraguay" | pais_region == "Brasil" | pais_region == "Venezuela") |> 
  filter (Anio >= 1998)
```
...
ggplot(industria_2018, aes(x=pais_region,y=valor, fill=industria_x_genero))+
  ...
  ggtitle ("Brecha de acceso a la industria en 2018") + 
  labs(x = "Paises Mercosur",y = "Porcentaje dentro de la fuerza laboral") 
```

\*Del grafico podemos leer que nuestra hipotesis se confirma: el peso del sector industrial como empleador de la fuerza laboral masculina en los paises del Mercosur es mucho mayor que en el caso de la fuerza laboral femenina. Entendemos que esto se debe al fenomeno llamado "segmentación vertical". Es decir, el hecho de que existan actividades economicas que emplean mayormente a varones, llamadas "masculinizadas" y actividades que emplean mayormente a mujeres, llamadas "feminizadas". 
Esta inserción laboral diferenciada se debe a los estereotipos de género, que determina que haya ciertos trabajos para mujeres y otros para varones. En este caso por ser la industria un trabajo asociado socialmente a los varones, es una actividad altamente masculinizada y por lo tanto las mujeres tienen un acceso mucho menor.
...
##Para contestar nuestra segunda pregunta queremos ver especificamente que sucede en los años de los extremos, con el objetivo de poder comparar el comportamiento de ambas variables y evaluar si hubo una reduccion o ampliacion de la brecha existentes. Por esta razon filtraremos los años 1998 y 2018 y calcularemos la variacion entre puntas
...
#A partir de la lectura del grafico podemos concluir que en el periodo 1998 - 2018 no solo no se achico la brecha de acceso a empleos en la industria sino que se agrando, esto se puede explicar por xxx


##prueba ro
```{r}
industria_1998_2018 <- select(empleo_mercosur, pais_region, Anio, empleo_industria_mujeres, empleo_industria_hombres)|>
...
para sacar tasa de variacion
```{r}
pruebaindustria_2018_1998 <- select(empleo_genero, variable, pais_region, "1998", "2018")|>
  filter(variable == "empleo_industria_hombres" | variable == "empleo_industria_mujeres")
```
